...
#     # Case 2 in Figure 9
#     np.random.seed(seed)
#     noise = np.load(f'../noise_{Num}.npy')[np.random.choice(np.arange(25), 13, replace=False)]
#     while True:
#         edge = [ToBij() for _ in range(12)]
#         adjacency = np.zeros([13, 13])
#         adjacency[1, 0] = edge[0]
#         adjacency[2, 0] = edge[1]
#         adjacency[3, 0] = edge[2]
#         adjacency[4, 1] = edge[3]
#         adjacency[5, 1] = edge[4]
#         adjacency[6, 1] = edge[5]
#         adjacency[7, 2] = edge[6]
#         adjacency[8, 2] = edge[7]
#         adjacency[9, 2] = edge[8]
#         adjacency[10, 3] = edge[9]
#         adjacency[11, 3] = edge[10]
#         adjacency[12, 3] = edge[11]
#         mixing = np.linalg.inv(np.eye(13) - adjacency)
#         if np.all(np.abs(mixing[np.abs(mixing) > 1e-6]) > 0.25): # faithfulness
#             break

#     L1=noise[0]*Toa()
#     L2=noise[1]*Toa()+edge[0]*L1
#     L3=noise[2]*Toa()+edge[1]*L1
#     L4=noise[3]*Toa()+edge[2]*L1
#     x1=noise[4]*Toa()+edge[3]*L2
#     x2=noise[5]*Toa()+edge[4]*L2
#     x3=noise[6]*Toa()+edge[5]*L2
#     x4=noise[7]*Toa()+edge[6]*L3
#     x5=noise[8]*Toa()+edge[7]*L3
#     x6=noise[9]*Toa()+edge[8]*L3
#     x7=noise[10]*Toa()+edge[9]*L4
#     x8=noise[11]*Toa()+edge[10]*L4
#     x9=noise[12]*Toa()+edge[11]*L4
